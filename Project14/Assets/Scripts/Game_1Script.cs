...
public class Game_1Script : MonoBehaviour
{
    ...
    private IEnumerator StartBlinking(float interval, int seqSize)
    {
        ...
        rightSequence = seq.Select(number => number + 1).ToList();  // Записываем последовательность в поле, для проверки далее. Индекс смещен, т.к. в Unity нумерация в названии блоков начинается с 1.
